...
Live R2 bucket proof has been verified with Wrangler for Cloudflare account `jami-studio`: bucket
`intercal` exists in location `ENAM` with default storage class `Standard`. This is bucket metadata
proof, not a fresh source-document object write/read smoke through the S3 adapter. Missing provider
telemetry is reported as unavailable, not guessed.
